...
export default function LoginPage() {
    ...
    const handleLogin = async (role: string, wallet: string) => {
        setLoading(true);
        try {
            const res = await api.post("/api/auth/login", { wallet, role });

            if (res.data?.token) {
                toast.success(`Logged in as ${role}`);
                // useAuth().login stores token as "satelink_token", calls /auth/me, sets user, and redirects by role
            ...
            <div className="max-w-md w-full p-8 rounded-3xl border border-white/10 bg-white/5 backdrop-blur-xl shadow-2xl">
                <div className="text-center mb-10">
                    ...
                    <p className="text-white/50 text-sm">
                        Select a diagnostic role to continue.
                    </p>
                </div>
                ...
                <div className="space-y-4">
                    <button
                        onClick={() =>
                            handleLogin("admin_super", "0xadmin_super")
                        }
                        disabled={loading}
                    ...
                    <button
                        onClick={() =>
                            handleLogin("node_operator", "0xnode_op_1")
                        }
                        disabled={loading}
                    ...
                    <button
                        onClick={() =>
                            handleLogin("builder", "0xbuilder_1")
                        }
                        disabled={loading}
                        className="w-full py-3 rounded-xl border border-white/20 hover:bg-white/5 transition-all text-sm font-medium text-white/90"
                    >
                    ...
                    <button
                        onClick={() =>
                            handleLogin("distributor_lco", "0xdist_1")
                        }
                        disabled={loading}
                        className="w-full py-3 rounded-xl border border-white/20 hover:bg-white/5 transition-all text-sm font-medium text-white/90"
                    >
                    ...
                    <button
                        onClick={() =>
                            handleLogin("enterprise", "0xent_1")
                        }
                        disabled={loading}
                        className="w-full py-3 rounded-xl border border-white/20 hover:bg-white/5 transition-all text-sm font-medium text-white/90"
                    >
