...
export default function RevenueEventsPage() {
    ...
    const fetch = useCallback(async () => {
        try {
            setError('');
            const res = await api.get('/admin/revenue/events?limit=200');
            if (res.data.ok) setData(res.data.events);
        } catch (e: any) {
    ...
    const columns = [
        ...
        { key: 'amount_usdt', label: 'Amount', render: (r: any) => <span className="font-mono text-sm text-emerald-400">${parseFloat(r.amount_usdt || 0).toFixed(4)}</span> },
    ...
    return (
        <div className="p-4 sm:p-6 lg:p-8 max-w-[1600px] mx-auto">
            <PageHeader title="Revenue Events" subtitle="Individual revenue event records"
            ...
            <Card className="bg-zinc-900/60 border-zinc-800/60">
                <CardContent className="p-0">
                    <DataTable columns={columns} data={data} loading={loading} searchable searchPlaceholder="Search events..." emptyMessage="No events" />
                </CardContent>
            </Card>
        </div>
    );
