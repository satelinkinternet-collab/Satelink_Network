...
export class GrowthEngine {
    ...
    async calculateNodeProfitability() {
        ...
        for (const n of nodes) {
            ...
            // Uptime %
            const uptimeRow = await this.db.prepare(
            ...
            let avgLatency = 0;
            try {
                const latRow = await this.db.prepare(
        ...
        for (const r of report) {
            try {
                await this.db.prepare(`
                    INSERT INTO node_profitability_daily 
                    ...
                    ON CONFLICT (date_key, node_id) DO UPDATE SET
                        revenue_generated = EXCLUDED.revenue_generated,
                        rewards_earned = EXCLUDED.rewards_earned,
                        op_count = EXCLUDED.op_count,
                        infra_cost_est = EXCLUDED.infra_cost_est,
                        uptime_pct = EXCLUDED.uptime_pct,
                        success_rate = EXCLUDED.success_rate,
                        avg_latency_ms = EXCLUDED.avg_latency_ms,
                        rank = EXCLUDED.rank,
                        net_profit = EXCLUDED.net_profit
                `).run([dateKey, r.node, r.revenue_generated, r.rewards_earned, r.op_count, r.infra_cost,
    ...
    async checkAttritionRisk() {
        ...
        const atRisk = await this.db.prepare(`
    ...
    async processCommissions() {
        ...
        const referrals = await this.db.prepare("SELECT * FROM conversions").all();

        for (const ref of referrals) {
            const uptime = await this.db.prepare(
                "SELECT uptime_seconds FROM node_uptime WHERE node_wallet = ? ORDER BY epoch_id DESC LIMIT 1"
            ).get([ref.node_id]);

            if (uptime && uptime.uptime_seconds > 0) {
                // Per-distributor rate (lookup or default 5%)
                const distConfig = await this.db.prepare(
                ...
                const dayAgo = now - 86400000;
                const nodeRev = await this.db.prepare(
                    "SELECT SUM(amount_usdt) as t FROM revenue_events_v2 WHERE node_id = ? AND created_at > ?"
                ).get([ref.node_id, dayAgo]);
                ...
                try {
                    await this.db.prepare(`
                        ...
                        VALUES (?, ?, ?, ?, 'accrued', 0, ?, ?, ?)
                    `).run([id, ref.wallet, `active_node_${ref.node_id}`, amount, rate, dailyCap, now]);
    ...
    async getCommissionsStats() {
        const total = await this.db.prepare("SELECT SUM(amount_usdt) as t FROM distributor_commissions WHERE fraud_flag = 0").get();
        const byDistributor = await this.db.prepare(`
    ...
    async detectCommissionFraud() {
        ...
        const selfRefs = await this.db.prepare("SELECT * FROM conversions WHERE wallet = node_id").all();
        for (const fraud of selfRefs) {
            ...
            await this.db.prepare(
                "UPDATE distributor_commissions SET fraud_flag = 1, fraud_score = GREATEST(fraud_score, 80) WHERE distributor_wallet = ?"
            ).run([fraud.wallet]);
        }

        // 2. Activation Spikes (> 10 nodes in 24h)
        const spikes = await this.db.prepare(`
        ...
        for (const s of spikes) {
            ...
            await this.db.prepare(
                "UPDATE distributor_commissions SET fraud_score = GREATEST(fraud_score, 50) WHERE distributor_wallet = ?"
            ).run([s.wallet]);
        }
    ...
    async trackAcquisition() {
        ...
        const dailyRev = (await this.db.prepare(
            "SELECT AVG(revenue_generated) as avg FROM node_profitability_daily WHERE date_key > ?"
        ).get([new Date(thirtyDaysAgo).toISOString().split('T')[0]]))?.avg || 0;
        const ltv = dailyRev * 180;

        // Churn: nodes NOT seen recently vs total
        const totalNodes = (await this.db.prepare("SELECT COUNT(*) as c FROM registered_nodes").get())?.c || 0;
    ...
    async analyzeUnitEconomics(period = '7d') {
        ...
        try {
            await this.db.prepare(`
                INSERT INTO unit_economics_daily
                ...
                ON CONFLICT (date_key) DO UPDATE SET
                    ...
                    total_nodes_active = EXCLUDED.total_nodes_active,
                    burn_rate = EXCLUDED.burn_rate,
                    avg_margin_percent = EXCLUDED.avg_margin_percent,
                    created_at = EXCLUDED.created_at
            `).run([dateKey, totalRev, totalRew, activeNodes, burnRate, margin, now]);
        } catch (e) { /* soft fail */ }
        ...
        if (burnRate > BURN_THRESHOLD) {
            ...
            try {
                await this.db.prepare(`
                    INSERT INTO system_config (key, value, updated_at) VALUES ('rewards_mode', 'SIMULATED', $1)
    ...
    async processTieredCommissions() {
        ...
        let referrals = [];
        try {
            referrals = await this.db.prepare("SELECT * FROM conversions").all() || [];
        ...
        for (const ref of referrals) {
            ...
            for (let tier = 1; tier <= MAX_DEPTH; tier++) {
                ...
                // Per-node daily cap
                const todayNodeTotal = await this.db.prepare(
                ...
                ).get([currentReferrer, `%${ref.node_id}%`, dayAgo]);
                ...
                try {
                    await this.db.prepare(`
                        ...
                        VALUES (?, ?, ?, ?, 'accrued', 0, ?, ?, ?, ?, ?, ?)
                    `).run([id, currentReferrer, `tier${tier}_node_${ref.node_id}`, amount, tierRate, dailyCap,
    ...
    async detectActivationSpike() {
        try {
            ...
            const recentActivations = (await this.db.prepare(
                "SELECT COUNT(*) as c FROM conversions WHERE created_at > ?"
            ).get([hourAgo]))?.c || 0;

            const avgHourly = (await this.db.prepare(
