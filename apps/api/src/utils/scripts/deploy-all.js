...
async function main() {
    ...
    // 1. Mock USDT (Conditional or always for MVP test)
    // For Production check network name, but for now deploy mock
    ...
    // Explicit call (sometimes deployment instance doesn't have methods in artifacts view? No, it should)
    // Just using the returned contract object:
    // const tx = await vault.contract.transferOwnership(claims.address); // if wrapper returned contract
    // But helper returns object {contract, address}. contract is ethers contract.
    const tx = await vault.contract.transferOwnership(claims.address);
    await tx.wait();
