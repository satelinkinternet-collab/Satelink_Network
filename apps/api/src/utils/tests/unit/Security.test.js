...
import { DemandBuffer } from "../../../../app_factory.mjs";
...
describe.skip("Security & Abuse Protection Test", function () {
  ...
  before(async function () {
    ...
    // Setup node
    await agent
      .post("/services/services/nodes/bootstrap-payment")
      .set("X-Admin-Key", ADMIN_API_KEY)
      .send({ nodeWallet, nodeType: "edge" });
  });
  ...
  it("should detect abuse (operation spike)", async function () {
    db.prepare(
      "INSERT INTO op_counts (epoch_id, user_wallet, op_type, ops, weight, created_at) VALUES (1, ?, 'api_relay_execution', 6000, 1.0, ?)"
    ...
    // expected to be blocked by abuse detection
    expect(res.status).to.equal(500);
    expect(res.body.error || "").to.include("Abuse");
  });

  it("should auto-pause withdrawals if treasury is unsafe", async function () {
    db.prepare(
      "INSERT INTO revenue_events (amount, token, source, created_at) VALUES (-1000, 'USDT', 'MALICIOUS_DRAIN', ?)"
