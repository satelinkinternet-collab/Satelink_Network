...
<<<<<<< HEAD:apps/api/src/gateway/routes/admin_control_room_api.js
        try {
          const limit = 50;
...
>>>>>>> integration/full-product:src/routes/admin_control_room_api.js
        }
      });
    ...
    router.get('/services/settlement/overview', requireAdmin, async (req, res) => {
        try {
    ...
    router.get('/services/settlement/batches', requireAdmin, async (req, res) => {
        try {
            const limit = parseIntParam(req.query.limit, 50);
            const offset = parseIntParam(req.query.offset, 0);
    ...
    router.post('/services/settlement/config', requireAdmin, async (req, res) => {
        try {
            const isSuper = req.user.role === 'admin_super';
    ...
    router.post('/services/settlement/evm/reconcile/:batch_id', requireAdmin, async (req, res) => {
        try {
            const engine = req.app.get('settlementEngine');
    ...
    router.post('/services/settlement/evm/retry-item', requireAdmin, async (req, res) => {
        try {
            const isSuper = req.user.role === 'admin_super';
            if (!isSuper) return res.status(403).json({ ok: false, error: "Super Admin required" });
