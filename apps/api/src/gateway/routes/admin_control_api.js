...
export function createAdminControlRouter(opsEngine, auditService = null) {
    ...
    router.get('/command/summary', async (req, res) => {
        try {
            ...
            const opsCount = (await db.prepare("SELECT COUNT(*) as c FROM revenue_events_v2 WHERE created_at > ?").get([fiveMinsAgo])).c;
            const activeNodes = (await db.prepare("SELECT COUNT(*) as c FROM nodes WHERE last_seen > ?").get([fiveMinsAgo])).c;

            // Revenue 24h
            const dayAgo = Math.floor(Date.now() / 1000) - 86400;
            const revenue24h = (await db.prepare("SELECT SUM(amount_usdt) as t FROM revenue_events_v2 WHERE created_at > ?").get([dayAgo])).t || 0;

            // Counts form monitoring tables
            const alertsOpen = (await db.prepare("SELECT COUNT(*) as c FROM security_alerts WHERE status = 'open'").get()).c;
            const errors1h = (await db.prepare("SELECT COUNT(*) as c FROM error_events WHERE last_seen_at > ?").get([Date.now() - 3600000])).c;
            const slowQueries1h = (await db.prepare("SELECT COUNT(*) as c FROM slow_queries WHERE last_seen_at > ?").get([Date.now() - 3600000])).c;
    ...
    router.get('/network/nodes', async (req, res) => {
        try {
            const nodes = await db.prepare("SELECT * FROM nodes ORDER BY last_seen DESC LIMIT 200").all();
    ...
    router.get('/ops/errors', async (req, res) => {
        try {
            const errors = await db.prepare("SELECT * FROM error_events ORDER BY last_seen_at DESC LIMIT 100").all();
    ...
    router.get('/ops/traces', async (req, res) => {
        try {
            const traces = await db.prepare("SELECT * FROM request_traces ORDER BY created_at DESC LIMIT 100").all();
    ...
    router.get('/security/alerts', async (req, res) => {
        try {
            const alerts = await db.prepare("SELECT * FROM security_alerts ORDER BY created_at DESC LIMIT 100").all();
    ...
    router.post('/security/alerts/:id/triage', async (req, res) => {
        ...
        await db.prepare("UPDATE security_alerts SET status = ?, resolution_notes = ? WHERE id = ?").run([status, notes, id]);
        res.json({ ok: true });
    });
    ...
    router.post('/controls/pause-withdrawals', async (req, res) => {
        ...
        await db.prepare("INSERT INTO system_flags (key, value, updated_by, updated_at) VALUES ('withdrawals_paused', ?, ?, ?) ON CONFLICT(key) DO UPDATE SET value=excluded.value, updated_by=excluded.updated_by, updated_at=excluded.updated_at").run(
