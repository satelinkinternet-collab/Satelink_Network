...
// Removed __test/state
    ...
    router.get("/services/integrations/node/:wallet", async (req, res) => {
        try {
            ...
            const uptime = await opsEngine.db.prepare("SELECT uptime_seconds, score FROM node_uptime WHERE node_wallet = ? AND epoch_id = ?").get([wallet, epochId]);
    ...
    router.get("/operations/epoch-stats", async (_req, res) => {
        try {
            ...
            const stats = await opsEngine.db.prepare(`
                SELECT 
            ...
            const ops = await opsEngine.db.prepare("SELECT COUNT(*) as count, COALESCE(SUM(amount_usdt), 0) as revenue FROM revenue_events WHERE epoch_id = ?").get([epochId]);
    ...
    router.post("/webhooks/moonpay", async (req, res) => {
        ...
        try {
            const existing = await opsEngine.db.prepare("SELECT 1 FROM payments_inbox WHERE provider = ? AND event_id = ?").get(["moonpay", eventId]);
            if (existing) {
            ...
            await opsEngine.db.transaction(async (tx) => {
                await tx.prepare("INSERT INTO payments_inbox (provider, event_id, status, payload_json, created_at) VALUES (?,?,?,?,?)").run(
                    ["moonpay", eventId, "processed", JSON.stringify(payload), now]);
                await tx.prepare("INSERT INTO revenue_events (amount, amount_usdt, token, source, payer_wallet, tx_ref, source_type, provider, epoch_id, created_at) VALUES (?,?,'USDT','INTEGRATION',?,?,?,?,?,?)").run(
    ...
    router.post("/webhooks/fuse", async (req, res) => {
        ...
        try {
            const existing = await opsEngine.db.prepare("SELECT 1 FROM payments_inbox WHERE provider = ? AND event_id = ?").get(["fuse", eventId]);
            if (existing) {
    ...
    router.post("/services/integrations/manual/revenue", async (req, res) => {
        ...
        try {
            ...
            await opsEngine.db.prepare("INSERT INTO revenue_events (amount, amount_usdt, token, source, payer_wallet, tx_ref, source_type, provider, epoch_id, created_at) VALUES (?,?,'USDT','INTEGRATION',?,?,?,?,?,?)").run(
    ...
    router.get("/epochs/current", async (_req, res) => {
        try {
            const epochId = await opsEngine.initEpoch();
            const epoch = await opsEngine.db.prepare("SELECT * FROM epochs WHERE id = ?").get([epochId]);
            res.json(epoch);
        } catch (e) { res.status(500).json({ error: e.message }); }
    });
    ...
    router.post("/withdraw", async (req, res) => {
        ...
        try {
            ...
            // Phase 6 Audit Fix: Verify record creation
            const record = await opsEngine.db.prepare("SELECT 1 FROM withdrawals WHERE id = ?").get([withdrawalId]);
            if (!record) throw new Error("Withdrawal record creation failed verification");

            res.json({ ok: true, id: withdrawalId, status: "PENDING" });
        } catch (e) { res.status(500).json({ error: e.message }); }
    });
    ...
    router.post("/withdrawals/:id/complete", adminAuth, async (req, res) => {
        ...
        try {
            const w = await opsEngine.db.prepare("SELECT * FROM withdrawals WHERE id = ?").get([id]);
            if (!w) return res.status(404).json({ error: "Not found" });

            await opsEngine.db.transaction(async (tx) => {
                await tx.prepare("UPDATE withdrawals SET status='COMPLETED', tx_hash=? WHERE id=?").run([tx_hash || "", id]);
                // Upsert/Update balance
                const current = await tx.prepare("SELECT amount_usdt FROM balances WHERE wallet = ?").get([w.wallet]);
                if (current) {
                    await tx.prepare("UPDATE balances SET amount_usdt = amount_usdt - ? WHERE wallet = ?").run([w.amount_usdt, w.wallet]);
                } else {
                    // Should not happen if withdrawal exists, but safety
                    await tx.prepare("INSERT INTO balances (wallet, amount_usdt) VALUES (?, ?)").run([w.wallet, -w.amount_usdt]);
                }
            });
            res.json({ ok: true });
        } catch (e) { res.status(500).json({ error: e.message }); }
