...
export class PartnerEngine {
    ...
    /** Get all partners */
    async getPartners() {
        return await this.db.prepare("SELECT partner_id, partner_name, wallet, status, rate_limit_per_min, revenue_share_percent, total_revenue, total_ops, created_at, updated_at FROM partner_registry ORDER BY created_at DESC").all([]);
    }
