...
node -e '
  (async () => {
    ...
    await db.query("INSERT INTO registered_nodes (wallet, last_heartbeat, active, management_type) VALUES ($1, $2, 1, $3)", 
        [managedWallet, now, "managed"]);
    
    // 2. Create Uptime for it
    await db.query("INSERT INTO node_uptime (node_wallet, epoch_id, uptime_seconds, score) VALUES ($1, $2, 3600, 3600)",
    ...
    await db.query("INSERT INTO revenue_events_v2 (epoch_id, op_type, node_id, client_id, amount_usdt, status, created_at) VALUES ($1, $2, $3, $4, $5, $6, $7)",
    ...
    // 5. Verify Earnings
    const reserve = await db.get("SELECT amount_usdt FROM epoch_earnings WHERE epoch_id = $1 AND role = $2", [epochId, "infra_reserve"]);
    const operator = await db.get("SELECT amount_usdt FROM epoch_earnings WHERE epoch_id = $1 AND role = $2 AND wallet_or_node_id = $3", [epochId, "node_operator", managedWallet]);

    console.log("Infra Reserve:", reserve?.amount_usdt);
    console.log("Operator Net:", operator?.amount_usdt);

    if (Number(reserve?.amount_usdt) === 5.0 && Number(operator?.amount_usdt) === 45.0) {
